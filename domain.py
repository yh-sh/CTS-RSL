...
from dataloader.dataloader import domain_generator
...
train_dl, valid_dl, test_dl = domain_generator(data_path, configs, training_mode, args.domain)
...
if "fine_tune" in training_mode or "ft_" in training_mode:
    ...
    if 'SupCon' not in training_mode:
        load_from = os.path.join(
            os.path.join(logs_save_dir, experiment_description, run_description,
                                  "self_supervised_" + args.domain + f"_seed_{SEED}",
                         "saved_models"))
    else:
        load_from =  os.path.join(logs_save_dir, experiment_description, run_description,
                                  "SupCon_" + args.domain + f"_seed_{SEED}", "saved_models")
...
if training_mode == "gen_pseudo_labels":
    ft_perc = "1p"
    load_from = os.path.join(
        os.path.join(experiment_log_dir, "saved_models"))
    chkpoint = torch.load(os.path.join(load_from, "ckp_last.pt"), map_location=device)
    pretrained_dict = chkpoint["model_state_dict"]
...
if "train_linear" in training_mode or "tl" in training_mode:
    if 'SupCon' not in training_mode:
        ...
        load_from = os.path.join(
            os.path.join(logs_save_dir, experiment_description, run_description,
                                  "SupCon_" + args.domain + f"_seed_{SEED}",
                         "saved_models"))
    chkpoint = torch.load(os.path.join(load_from, "ckp_last.pt"), map_location=device)
    pretrained_dict = chkpoint["model_state_dict"]
    model_dict = model.state_dict()
