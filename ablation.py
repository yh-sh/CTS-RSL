...
from dataloader.dataloader import abaltion_generator
from models.TC import TC
from models.model import base_Model
from trainer.atrainer import Trainer, model_evaluate, gen_pseudo_labels
from utils import _calc_metrics, copy_Files
from utils import _logger, set_requires_grad
from dataloader.augmentations import SpatialTransform
...
parser.add_argument('--ablation',    default='Par_Tem', type=str, 
                    help='choice: Par, Par_Per, Par_Sca, Par_Jit, Tem, Tem_Per, Tem_Sca, Tem_Jit, Sca_Jit')
parser.add_argument('--selected_dataset',           default='HAR',              type=str,   help='Dataset of choice: sleepEDF, HAR, Epilepsy')
...
experiment_description = "{}_experiments".format(args.selected_dataset)
...
experiment_log_dir = os.path.join(logs_save_dir, experiment_description, run_description,
                                  training_mode + "_" + ablation + f"_seed_{SEED}")
...
train_dl, valid_dl, test_dl = abaltion_generator(data_path, configs, training_mode, ablation)
logger.debug("Data loaded ...")

# Load Model
model = base_Model(configs).to(device)
temporal_contr_model = TC(configs, device).to(device)
spatial = SpatialTransform(configs).to(device)

if "fine_tune" in training_mode or "ft_" in training_mode:
    # load saved model of this experiment
    if 'SupCon' not in training_mode:
        load_from = os.path.join(
            os.path.join(logs_save_dir, experiment_description, run_description, "self_supervised" + "_" + ablation + f"_seed_{SEED}",
                         "saved_models"))
    else:
        load_from = os.path.join(
            os.path.join(logs_save_dir, experiment_description, run_description, "SupCon" + "_" + ablation + f"_seed_{SEED}",
                         "saved_models"))
...
model_optimizer = torch.optim.Adam(model.parameters(), lr=configs.lr, betas=(configs.beta1, configs.beta2),
                                   weight_decay=3e-4)
spatial_optimizer = torch.optim.Adam(spatial.parameters(), lr=configs.lr, betas=(configs.beta1, configs.beta2),
                                   weight_decay=3e-1)
...
# Trainer
Trainer(model, spatial, temporal_contr_model, model_optimizer, spatial_optimizer, temporal_contr_optimizer, train_dl, valid_dl, test_dl, device,
        logger, configs, experiment_log_dir, training_mode, ablation)
