...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device,
    ...
    for epoch in range(1, config.num_epoch + 1):
        ...
        if (training_mode != "TS-TCC") and (training_mode != "CPC") and (training_mode != "SimCLR"):
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config,
    ...
    for batch_idx, (data, labels, aug1, aug2) in enumerate(train_loader):
        ...
        if training_mode == "CPC" or training_mode == "TS-TCC":
        ...
        elif training_mode == "CPC":

            loss = temp_cont_loss1

        elif training_mode == "SimCLR":
            predictions1, features1 = model(aug1)
            predictions2, features2 = model(aug2)
            _, temp_cont_feat1, temp_cont_feat2 = temporal_contr_model(features1, features2)
            loss = nt_xent_criterion(temp_cont_feat1, temp_cont_feat2)
    ...
    if (training_mode == "TS-TCC") or ( training_mode == "CPC") or ( training_mode == "SimCLR"):
        total_acc = 0
    else:
