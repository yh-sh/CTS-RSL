...
from dataloader.dataloader import cdomain_generator
from models.TC import TC
from models.model import base_Model
from trainer.ctrainer import Trainer, model_evaluate, gen_pseudo_labels
...
parser.add_argument('--training_mode',              default='TS-TCC',  type=str,
                    help='Modes of choice: SimCLR, CPC, TS-TCC')

parser.add_argument('--selected_dataset',           default='domain',              type=str,   help='Dataset of choice: sleepEDF, HAR, Epilepsy')
parser.add_argument('--data_path',                  default=r'data/',           type=str,   help='Path containing dataset')
parser.add_argument('--domain',                  default='a-b',           type=str)
...
experiment_description = "domain_experiments"
data_type = args.selected_dataset
training_mode = args.training_mode
run_description = args.run_description
train_set = args.domain[0]
test_set = args.domain[2]
...
experiment_log_dir = os.path.join(logs_save_dir, experiment_description, run_description,
                                  training_mode + "_" + args.domain + f"_seed_{SEED}")
...
train_dl, valid_dl, test_dl = cdomain_generator(data_path, configs, training_mode, args.domain)
...
if "fine_tune" in training_mode or "ft_" in training_mode:
    ...
    if 'SimCLR' in training_mode:
        load_from = os.path.join(
            os.path.join(logs_save_dir, experiment_description, run_description, "SimCLR_" + args.domain + f"_seed_{SEED}",
                         "saved_models"))
    elif 'CPC' in training_mode:
        load_from = os.path.join(
            os.path.join(logs_save_dir, experiment_description, run_description, "CPC_" + args.domain + f"_seed_{SEED}", "saved_models"))
    elif 'TS-TCC' in training_mode:
        load_from = os.path.join(
            os.path.join(logs_save_dir, experiment_description, run_description, "TS-TCC_" + args.domain + f"_seed_{SEED}", "saved_models")) 
...
if training_mode == "TS-TCC" or training_mode == "SimCLR" or training_mode == "CPC":  # to do it only once
